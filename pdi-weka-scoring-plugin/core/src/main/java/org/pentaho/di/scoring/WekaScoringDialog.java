...
public class WekaScoringDialog extends BaseStepDialog implements StepDialogInterface {
  ...
  public String open() {
    ...
    m_wlStepname
        .setText( BaseMessages.getString( WekaScoringMeta.PKG, "WekaScoringDialog.StepName.Label" ) ); //$NON-NLS-1$
    ...
    m_wFileTab
        .setText( BaseMessages.getString( WekaScoringMeta.PKG, "WekaScoringDialog.FileTab.TabTitle" ) ); //$NON-NLS-1$
    ...
    m_wlFilename
        .setText( BaseMessages.getString( WekaScoringMeta.PKG, "WekaScoringDialog.Filename.Label" ) ); //$NON-NLS-1$
    ...
    saveModelMetaLab.setText(
        BaseMessages.getString( WekaScoringMeta.PKG, "WekaScoringDialog.SaveModelToMeta.Label" ) ); //$NON-NLS-1$
    ...
    m_wAcceptFileNameFromFieldCheckLab.setText( BaseMessages
        .getString( WekaScoringMeta.PKG, "WekaScoringDialog.AcceptFileNamesFromFieldCheck.Label" ) ); //$NON-NLS-1$
    ...
    m_wAcceptFileNameFromFieldTextLab.setText( BaseMessages
        .getString( WekaScoringMeta.PKG, "WekaScoringDialog.AcceptFileNamesFromField.Label" ) ); //$NON-NLS-1$
    ...
    m_wFieldsTab
        .setText( BaseMessages.getString( WekaScoringMeta.PKG, "WekaScoringDialog.FieldsTab.TabTitle" ) ); //$NON-NLS-1$
    ...
    m_wModelTab
        .setText( BaseMessages.getString( WekaScoringMeta.PKG, "WekaScoringDialog.ModelTab.TabTitle" ) ); //$NON-NLS-1$
    ...
    m_wbFilename.addSelectionListener( new SelectionAdapter() {
      @Override public void widgetSelected( SelectionEvent e ) {
        ...
        if ( XStream.isPresent() ) {
          ...
          filterNames[0] =
              BaseMessages.getString( WekaScoringMeta.PKG, "WekaScoringDialog.FileType.ModelFileBinary" ); //$NON-NLS-1$
          extensions[1] = "*.xstreammodel"; //$NON-NLS-1$
          filterNames[1] =
              BaseMessages.getString( WekaScoringMeta.PKG, "WekaScoringDialog.FileType.ModelFileXML" ); //$NON-NLS-1$
          extensions[2] = "*.xml"; //$NON-NLS-1$
          filterNames[2] =
              BaseMessages.getString( WekaScoringMeta.PKG, "WekaScoringDialog.FileType.ModelFilePMML" ); //$NON-NLS-1$
  ...
  private boolean loadModel() {
    ...
    try {
      if ( !Const.isEmpty( filename ) && WekaScoringData.modelFileExists( filename, transMeta ) ) {

        WekaScoringModel tempM = WekaScoringData.loadSerializedModel( filename, log, transMeta );
      ...
      ex.printStackTrace();
      log.logError( BaseMessages.getString( WekaScoringMeta.PKG, "WekaScoringDialog.Log.FileLoadingError" ),
          ex ); //$NON-NLS-1$
    }
  ...
  private void mappingString( WekaScoringModel model ) {
    ...
    try {
      ...
      if ( stepMetaTemp != null ) {
        ...
        String
            headerS =
            BaseMessages.getString( WekaScoringMeta.PKG, "WekaScoringDialog.Mapping.ModelAttsHeader" ); //$NON-NLS-1$
        ...
        sep += "\t    ----------------\n"; //$NON-NLS-1$
        headerS += "\t    " //$NON-NLS-1$
            + BaseMessages.getString( WekaScoringMeta.PKG, "WekaScoringDialog.Mapping.IncomingFields" )
            + "\n"; //$NON-NLS-1$ //$NON-NLS-2$
        ...
        for ( int i = 0; i < header.numAttributes(); i++ ) {
          ...
          if ( temp.isNumeric() ) {
            attName +=
                BaseMessages.getString( WekaScoringMeta.PKG, "WekaScoringDialog.Mapping.Numeric" )
                    + ")"; //$NON-NLS-1$ //$NON-NLS-2$
          } else if ( temp.isNominal() ) {
            attName +=
                BaseMessages.getString( WekaScoringMeta.PKG, "WekaScoringDialog.Mapping.Nominal" )
                    + ")"; //$NON-NLS-1$ //$NON-NLS-2$
          } else if ( temp.isString() ) {
            attName +=
                BaseMessages.getString( WekaScoringMeta.PKG, "WekaScoringDialog.Mapping.String" )
                    + ")"; //$NON-NLS-1$ //$NON-NLS-2$
          }
          ...
          if ( mappings[i] == WekaScoringData.NO_MATCH ) {
            ...
            result.append(
                inFieldNum + BaseMessages.getString( WekaScoringMeta.PKG, "WekaScoringDialog.Mapping.MissingNoMatch" )
                    + "\n" ); //$NON-NLS-1$ //$NON-NLS-2$
            ...
            result.append( inFieldNum + BaseMessages
                .getString( WekaScoringMeta.PKG, "WekaScoringDialog.Mapping.MissingTypeMismatch" )
                + "\n" ); //$NON-NLS-1$ //$NON-NLS-2$
          } else {
            ...
            if ( tempField.isBoolean() ) {
              fieldName +=
                  BaseMessages.getString( WekaScoringMeta.PKG, "WekaScoringDialog.Mapping.Boolean" )
                      + ")"; //$NON-NLS-1$ //$NON-NLS-2$
      ...
      log.logError(
          BaseMessages.getString( WekaScoringMeta.PKG, "WekaScoringDialog.Log.UnableToFindInput" ) ); //$NON-NLS-1$
      return;
    }
  }
  ...
  public void getData() {
    ...
    if ( m_currentMeta.getFileNameFromField() ) {
      ...
      if ( !Const.isEmpty( m_currentMeta.getFieldNameToLoadModelFrom() ) ) {
        m_wAcceptFileNameFromFieldText.setText( m_currentMeta.getFieldNameToLoadModelFrom() );
      }
      m_wAcceptFileNameFromFieldText.setEnabled( true );

      m_wCacheModelsCheckBox.setSelection( m_currentMeta.getCacheLoadedModels() );
      m_wlFilename
          .setText( BaseMessages.getString( WekaScoringMeta.PKG, "WekaScoringDialog.Default.Label" ) ); //$NON-NLS-1$
    }
    ...
    m_storeModelInStepMetaData.setSelection( m_currentMeta.getStoreModelInStepMetaData() );

    // Grab model if it is available (and we are not reading model file
    // names from a field in the incoming data
    WekaScoringModel
        tempM =
        ( m_currentMeta.getFileNameFromField() ) ? m_currentMeta.getDefaultModel() : m_currentMeta.getModel();
    if ( tempM != null ) {
      m_wModelText.setText( tempM.toString() );
  ...
  private void ok() {
    ...
    m_currentMeta.setStoreModelInStepMetaData( m_storeModelInStepMetaData.getSelection() );

    if ( !Const.isEmpty( m_wFilename.getText() ) && !m_currentMeta.getStoreModelInStepMetaData() ) {
