...
public class WekaScoringMeta 
  ...
  public String getXML() {
    ...
    if (m_fileNameFromField) {
      ...
      if (!Const.isEmpty(m_fieldNameToLoadModelFrom)) {
        ...
        m_log.logBasic("[WekaScoringMeta] ", 
            Messages.getString("WekaScoringMeta.Log.ModelSourcedFromField")
            + " " + m_fieldNameToLoadModelFrom);
      }
    }
    ...
    if (temp != null 
      ...
      try {
        ...
        m_log.logBasic("[WekaScoringMeta] ", "Serializing " + modType + " model.");
  ...
  public void loadXML(Node stepnode, 
    ...
    if (nrModels > 0) {
      ...
      try {
        ...
        String modType = (m_fileNameFromField) ? "default" : "";
        m_log.logBasic("[WekaScoringMeta] ", "Deserializing " + modType + " model.");
  ...
  protected void loadModelFile() throws Exception {
    ...
    if (modelFile.exists()) {
      if (m_fileNameFromField) {
        m_log.logBasic("[WekaScoringMeta] ", "loading default model from file.");
        m_defaultModel = WekaScoringData.loadSerializedModel(modelFile);
      } else {
        m_log.logBasic("[WekaScoringMeta] ", "loading model from file.");
