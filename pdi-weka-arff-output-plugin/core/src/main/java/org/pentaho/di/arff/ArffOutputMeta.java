 ...
 * @version 1.0
 */
@Step( id = "ArffOutput", image = "ARFF.svg", name = "Arff output", description = "Writes data in ARFF format to a "
  + "file",
  documentationUrl = "https://pentaho-community.atlassian.net/wiki/display/EAI/ARFF+Output", categoryDescription = "Data Mining" )
public class ArffOutputMeta extends BaseStepMeta implements StepMetaInterface {
   ...
   * @param info     The input rows metadata that enters the step through the specified channels in the same order as in
   *                 method getInfoSteps(). The step metadata can then choose what to do with it: ignore it or not.
   * @param nextStep if this is a non-null value, it's the next step in the transformation. The one who's asking, the
   *                 step where the data is targetted towards.
